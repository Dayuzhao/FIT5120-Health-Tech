...
`tracks` table, so GET /api/v1/tracks (added by the backend owner) serves the
Epic 6 / US6 background-music player from the hosted database instead of a
committed JSON file.

Re-runnable at any time: upserts on jamendo_id.
...
def main() -> None:
    ...
    with psycopg.connect(DATABASE_URL) as connection:
        ...
        print(f"Upserted {len(rows)} tracks; tracks table now holds {count} rows")
